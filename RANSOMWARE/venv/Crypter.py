def change_files(filename, cryptoFn, block_sizes=16):
    with open(filename, 'r+b') as _file:
        ...
        while raw_value:
            ...
            if len(raw_value) != len(cipher_value):
                raise ValueError('O valor cifrado {} tem um tamanho diferente do valor plano {len(cipher_value), len(raw_value)}')
